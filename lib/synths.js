// Built-in sounds, synthesized live with the Web Audio API.
// These let the board work with ZERO configuration. Once you upload your
// own clips to Firebase, the app loads those instead (see Soundboard.jsx).
...
export const SYNTHS = [
  { id: "airhorn", name: "Airhorn", category: "Memes", hotkey: "1", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      o.start(t); o.stop(t + 0.62); return [o];
    } },
  { id: "boing", name: "Boing", category: "Memes", hotkey: "2", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      o.start(t); o.stop(t + 0.46); return [o, lfo];
    } },
  { id: "bruh", name: "Bruh", category: "Memes", hotkey: "3", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      o.start(t); o.stop(t + 0.36); return [o];
    } },
  { id: "ding", name: "Ding", category: "Alerts", hotkey: "4", dur: "0:01",
    make: (ctx, out, t) => {
      const nodes = [];
      [[1, 0.5], [2.01, 0.28], [3.0, 0.14]].forEach(([m, a]) => {
        ...
        o.start(t); o.stop(t + 1.25); nodes.push(o);
      });
      return nodes;
    } },
  { id: "coin", name: "Coin", category: "Game", hotkey: "5", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      o.start(t); o.stop(t + 0.34); return [o];
    } },
  ...
  { id: "sadtrombone", name: "Sad Trombone", category: "Alerts", hotkey: "9", dur: "0:02",
    make: (ctx, out, t) => {
      const seq = [311, 277, 247, 196]; let s = t; const nodes = [];
      seq.forEach((f, i) => {
  ...
  { id: "tada", name: "Ta-da", category: "Alerts", hotkey: "0", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      [523, 659, 784, 1047].forEach((f) => {
        ...
        o.start(t); o.stop(t + 0.92); nodes.push(o);
      });
      return nodes;
    } },
  { id: "pop", name: "Pop", category: "Game", hotkey: "Q", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      o.start(t); o.stop(t + 0.1); return [o];
    } },
  ...
  { id: "whistle", name: "Whistle", category: "FX", hotkey: "E", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      o.start(t); o.stop(t + 0.55); return [o, lfo];
    } },
  { id: "riser", name: "Riser", category: "FX", hotkey: "R", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      g.gain.exponentialRampToValueAtTime(0.0001, t + 1.2);
      ns.start(t); ns.stop(t + 1.2); return [ns];
    } },
  { id: "sparkle", name: "Sparkle", category: "FX", hotkey: "T", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        o.start(s); o.stop(s + 0.13); nodes.push(o);
      }
      return nodes;
    } },
  { id: "heartbeat", name: "Heartbeat", category: "Alerts", hotkey: "Y", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      [t, t + 0.28].forEach((s, i) => {
        ...
        o.start(s); o.stop(s + 0.2); nodes.push(o);
      });
      return nodes;
    } },
  { id: "click", name: "Click", category: "FX", hotkey: "U", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      ns.start(t); ns.stop(t + 0.03); return [ns];
    } },
  { id: "wow", name: "Wow", category: "Memes", hotkey: "I", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      o.start(t); o.stop(t + 0.46); return [o];
    } },
  { id: "vibe", name: "Vibe Pad", category: "Memes", hotkey: "O", dur: "0:02",
    make: (ctx, out, t) => {
      ...
      [220, 261.6, 329.6].forEach((f) => {
        ...
        o.start(t); o.stop(t + 1.72); nodes.push(o);
      });
      return nodes;
    } },
  { id: "buzzer", name: "Buzzer", category: "Alerts", hotkey: "P", dur: "0:01",
    make: (ctx, out, t) => {
      ...
      [110, 110.8].forEach((f) => {
        ...
        o.start(t); o.stop(t + 0.62); nodes.push(o);
      });
      return nodes;
    } },
];
